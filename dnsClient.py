...
def dns_query(type, name, server):
    ...
    server_address = (server, 53)  # DNS runs on port 53
    ...
    ARCOUNT = 0
        ...
        # Bit positions in the 16-bit FLAGS word (second field):
        # Bit 15: QR
    ...
    qname_parts = name.split('.')  # Split on dots
    qname_encoded_parts = [struct.pack('B', len(part)) + part.encode('ascii') for part in qname_parts]
    qname_encoded = b''.join(qname_encoded_parts) + b'\x00'  # Null byte terminates the QNAME

    # Encode the QTYPE and QCLASS

    if type == 'A':
        qtype = 1      # RFC 1035: A = 1 (IPv4 address)
    elif type == 'AAAA':
        qtype = 28     # RFC 3596: AAAA = 28 (IPv6 address)
    else:
        raise ValueError('Invalid type')

    qclass = 1  # IN (Internet) class = 1
    ...
    response_header = data[:12]  # DNS header is always 12 bytes
    ...
    response_question = data[12:12+len(question)]  # Start right after the 12-byte header
    assert response_question == question

    # Parse the response answer section
    response_answer = data[12+len(question):]  # Start right after header + question
    offset = 0
    for _ in range(ANCOUNT):
        ...
        offset += 10  # 2 (type) + 2 (class) + 4 (TTL) + 2 (rdlength) = 10 bytes
